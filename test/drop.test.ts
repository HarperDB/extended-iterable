		...
		if (hasSyncTests(testData)) {
			describe(`${name} sync`, () => {
				...
				if (testData.syncNextThrows) {
					...
					it('should throw an error if the iterator next() throws an error with async transformer', async () => {
						assert(testData.syncNextThrows);
						const data = testData.syncNextThrows();
						const iter = new ExtendedIterable(data, async (value) => value * 2);
						const dropIter = iter.drop(2)[Symbol.iterator]();
					...
					it.only('should reject if the iterator next() throws an error at a specific index with async transformer', async () => {
						assert(testData.syncNextThrows);
						const data = testData.syncNextThrows(3);
						const iter = new ExtendedIterable(data, async (value) => value * 2);
						const dropIter = iter.drop(3)[Symbol.iterator]();
